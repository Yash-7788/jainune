...
async def get_current_user_for_logout(
    ...
    """Authentication dependency specifically for session logout: permits banned or suspended users to revoke tokens and terminate sessions."""
    payload = await validate_access_token(credentials, redis)
    user_id = payload["sub"]
...
CurrentUserForLogout = Annotated[dict, Depends(get_current_user_for_logout)]
